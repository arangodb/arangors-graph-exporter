...
#[tokio::test]
#[serial]
async fn test_custom_aql_separate_queries() {
    setup(true).await;
    let db_config = build_db_config();
    let load_config = build_load_config();
...
#[tokio::test]
#[serial]
async fn test_custom_aql_separate_queries_with_filtering() {
    setup(true).await;
    let db_config = build_db_config();
    let load_config = build_load_config();
    ...
    let handle_vertices = move |vertex_ids: &Vec<Vec<u8>>,
                                _columns: &mut Vec<Vec<Value>>,
                                _vertex_field_names: &Vec<String>| {
        assert_eq!(vertex_ids.len(), 5); // Should be 5 vertices (indices 5-9)
        Ok(())
    };
    let vertices_result = graph_loader.do_vertices(handle_vertices).await;
    assert!(vertices_result.is_ok());
...
#[tokio::test]
#[serial]
async fn test_custom_aql_combined_query() {
    setup(true).await;
    let db_config = build_db_config();
    let load_config = build_load_config();
    ...
    let custom_queries = CustomAqlQueries::new_combined(combined_query, None);
    let graph_loader_res = load_with_custom_aql(db_config, load_config, custom_queries).await;
...
#[tokio::test]
#[serial]
async fn test_custom_aql_with_bind_vars() {
    setup(true).await;
    let db_config = build_db_config();
    let load_config = build_load_config();
    ...
    let custom_queries = CustomAqlQueries::new_separate(vertex_query, edge_query, Some(bind_vars));
    let graph_loader_res = load_with_custom_aql(db_config, load_config, custom_queries).await;
    ...
    let handle_vertices = move |vertex_ids: &Vec<Vec<u8>>,
                                _columns: &mut Vec<Vec<Value>>,
                                _vertex_field_names: &Vec<String>| {
        assert_eq!(vertex_ids.len(), 6); // x >= 5 means indices 4-9 (x values 5,6,7,8,9,10)
        Ok(())
    };
    let vertices_result = graph_loader.do_vertices(handle_vertices).await;
    assert!(vertices_result.is_ok());
...
#[tokio::test]
#[serial]
async fn test_custom_aql_invalid_query() {
    setup(true).await;
    let db_config = build_db_config();
    let load_config = build_load_config();

    // Invalid query that will fail
    let vertex_query = "FOR v IN nonexistent_collection RETURN v".to_string();
    let edge_query = format!("FOR e IN {} RETURN e", EDGE_COLLECTION);
